...
/* ************************************************************************* */
    ...
    // invisible middle of gunSight, really evaluated
    var realTarget = null;
    ...
    // array (template) definitions of bandits (structured)
    var tmpDefs = new Array();
...
/* ************************************************************************* */
    ...
    function ini(evt){
        ...
        gunSight = SVGDocument.getElementById("gunSight");
        realTarget = SVGDocument.getElementById("theMiddle");
        healthMask = SVGDocument.getElementById("healthMask");
        activeElements = SVGDocument.getElementById("activeElements");
        // generate letters
        abeceda();
    }
    ...
    function init(evt){
        ...
        while (activeElements.getChildNodes().length>0){
            activeElements.removeChild(activeElements.getFirstChild());
        }
        ...
        history = new Array();
        score = 0;
        svgDocument.getElementById('score').firstChild.data = 'Score: ' + score;
        health = 100;
        healthMask.setAttributeNS(null, "x", 592);
        gameRunning = true;
        ...
        SVGDocument.getElementById("hideStartScreen").beginElement();
    ...
    function invokeInnocent(){
        ...
        if (current != -1){
            ...
            svgDocument.getElementById(innocents[current]['name']+'ShowDialog').beginElement();
    ...
    function disableInnoBonus(whichOne, byGun){
        ...
        if (byGun){
            ...
            if (innocents[whichOne]['bonus']){
                svgDocument.getElementById(innocents[whichOne]['name']+'HideDialog').beginElement();
                svgDocument.getElementById(innocents[whichOne]['name']+'Killed').beginElement();
                innocents[whichOne]['living'] = 0;
                score += 50;    // add score to player
                svgDocument.getElementById('score').firstChild.data = 'Score: ' + score;
                svgDocument.getElementById('hit3').beginElement();
            ...
            // oh man, you killed me
                ...
                svgDocument.getElementById('score').firstChild.data = 'Score: ' + score;
                svgDocument.getElementById('hit2').beginElement();
            }
    ...
    // handling mouse/gunSight moving
    function DoMouseMove(evt){
        if (!gameRunning)
            return;

        if (quality){
            gunSight.setAttributeNS(null, "x", evt.clientX-40);      // 40
            gunSight.setAttributeNS(null, "y", evt.clientY-40);
            realTarget.setAttributeNS(null, "cx", evt.clientX-20);   // 20
            realTarget.setAttributeNS(null, "cy", evt.clientY-20);
        } else {
            realTarget.setAttributeNS(null, "cx", evt.clientX);      // 0
            realTarget.setAttributeNS(null, "cy", evt.clientY);
        }
            //collisionBB('cowboy','theMiddle',false);
    }
    ...
    function DoMouseDown(evt, emulate){
        ...
        var btn = (evt) ? evt.getButton() : emulate;

        // shoot if bullets are loaded
        if ((btn==0) && (bullets>0)){
            svgDocument.getElementById('ammo'+bullets).setAttributeNS(null, "opacity", 0.4);
            if (quality)
                svgDocument.getElementById("rotateMagazine").beginElement();
            // fire's sound
            svgDocument.getElementById('gun_fire').beginElement();
            bullets--;
            ...
            for (var i=(bandits.length-1); i>=0; i--){
                if ( collisionBB(bandits[i].banditNode) ){
            ...
            for (var i=0; i<innocents.length; i++){
                if(innocents[i]['living'] && collisionBB(svgDocument.getElementById(innocents[i]['name']+'Act'))){
            ...
            if ((btn==1) && (bullets<1)){
                ...
                // nothing
                svgDocument.getElementById('emptyGun').beginElement();
            }
        }
    }
    ...
    function DoMouseUp(evt){
        ...
        if (quality){
            gunSight.setAttributeNS(null, "fill", 'black');
            gunSight.setAttributeNS(null, "stroke", 'black');
        }
    }
    ...
    function loadMagazine(){
        bullets = 6;
        for (var i = 1; i <= 6; i++)
            svgDocument.getElementById('ammo'+i).setAttributeNS(null, "opacity", 1);
        if (quality){
            svgDocument.getElementById('reload').beginElement();
            svgDocument.getElementById('gun_reload').beginElement();
            svgDocument.getElementById("reloadMagazine").beginElement();
        } else {
            svgDocument.getElementById('gun_reload').beginElement();
        }
    }
    ...
    function collisionBB(object){

        var bb1 = object.getBBox();
        var bb2 = realTarget.getBBox();
        ...
        // test y axis
        if ( ((bb1.y>=bb2.y) && (bb1.y<=(bb2.y+bb2.height))) || 
    ...
    function addBandit (){
        ...
        while (rightBandit==false){
            ...
            if (history.length<4){
                var comparing = history;
            } else {
                var comparing = new Array();
                for (var i = 0; i < 4; i++){
                    comparing[i] = history[(history.length - i - 1)];
                }
            }
        ...
        // keep history
        history[history.length] = genType;
    ...
    function bandit (id, type){
        ...
        this.banditNode = svgDocument.createElementNS(svgns, "rect");
        ...
        activeElements.appendChild(this.banditNode);
        ...
        activeElements.appendChild(this.banditPicture);
    }
    ...
    bandit.prototype.applyTemplate = function (x, y){
        this.banditPicture = svgDocument.getElementById(tmpDefs[this.type]['template']).cloneNode(true);
        this.banditPicture.setAttributeNS(null, "x", x);
        this.banditPicture.setAttributeNS(null, "y", y);
        // mask
        if (tmpDefs[this.type]['mask']!='')
            this.banditPicture.setAttributeNS(null, "mask", "url(#"+tmpDefs[this.type]['mask']+")");
        ...
        var ani = svgDocument.createElementNS(svgns, tmpDefs[this.type]['appear']['elem']);
        ...
        ani.setAttributeNS(null, "begin", document.documentElement.getCurrentTime());
        ani.setAttributeNS(null, "end", "indefinite");
        // add listener for shooting
        // ani.addEventListener("onend", doFire, false);
        // temporarily replaced by direct calling
        ...
        if (tmpDefs[this.type]['appear']['audio'])
            svgDocument.getElementById(tmpDefs[this.type]['appear']['audio']).beginElement();
        return ani;
    }
    ...
    bandit.prototype.killed = function (){
        ...
        this.banditPicture.setAttributeNS(null, "mask", "");
        ...
        // prepare temporary modifier according to the animation type
        var ani = svgDocument.createElementNS(svgns, tmpDefs[this.type]['die']['elem']);
        ...
        ani.setAttributeNS(null, "end", "indefinite");
/*
        var ani2 = svgDocument.createElementNS(svgns, 'animateTransform');
        ani2.setAttributeNS(null, "attributeName", 'transform');
        ani2.setAttributeNS(null, "type", 'scale');
        ani2.setAttributeNS(null, "dur", ''+tmpDefs[this.type]['die']['dur']/2+'ms');
        ani2.setAttributeNS(null, "from", '1 1');
        ani2.setAttributeNS(null, "to",   '1 0.5');
        ani2.setAttributeNS(null, "fill", "freeze");
        ani2.setAttributeNS(null, "repeatCount", 1);
        ani2.setAttributeNS(null, "begin", document.documentElement.getCurrentTime());
        ani2.setAttributeNS(null, "end", "indefinite");
        this.banditPicture.appendChild(ani2);
*/
        //this.banditPicture.setAttribute("transform", "scale(1 0.5) translate(0 150)");

        //alert(printNode(ani));
        // play SFX, if available
        if (tmpDefs[this.type]['die']['audio'])
            svgDocument.getElementById(tmpDefs[this.type]['die']['audio']).beginElement();
        ...
        for (var i=0; i<bandits.length; i++){
            if ( bandits[i].id==id ){
                ...
                // negative one
                    ...
                    bandits[i].firing = svgDocument.getElementById(tmpDefs[bandits[i].type]['fire']['template']).cloneNode(true);
                    ...
                    //alert(printNode(bandits[i].firing));
                    activeElements.appendChild(bandits[i].firing);
    
                    // show blood
                    svgDocument.getElementById('hit').beginElement();
                    // decrement health
                    health -= 10;
                    healthMask.setAttributeNS(null, "x", healthMask.getAttribute("x")-10);
    
                    if (tmpDefs[bandits[i].type]['fire']['audio'])
                        svgDocument.getElementById(tmpDefs[bandits[i].type]['fire']['audio']).beginElement();
                ...
                // positive one
                    ...
                    var ani = svgDocument.createElementNS(svgns, tmpDefs[bandits[i].type]['appear']['elem']);
    ...
    function gameOver(){
        ...
        SVGDocument.getElementById("finalScore").firstChild.data = score;

        SVGDocument.getElementById("showDeathScreen").beginElement();
    ...
    function setQuality(event, fromInit){
        if (!SVGDocument) 
            SVGDocument = event.getTarget().getOwnerDocument();
        ...
        if (quality){
            SVGDocument.getElementById("q_SKY").setAttributeNS(null, "fill", "url(#sky_grad)");
            SVGDocument.getElementById("q_STREET").setAttributeNS(null, "fill", "url(#street_grad)");
            SVGDocument.getElementById("q_HEALTH").setAttributeNS(null, "fill", "url(#health_grad)");
            SVGDocument.getElementById("q_BABKA").beginElement();
            SVGDocument.getElementById("q_SUN").beginElement();
            SVGDocument.getElementById("q_FILLTOMADLO").setAttributeNS(null, "fill", "url(#madlo_ver)");
            SVGDocument.getElementById("q_MADLOHOR").setAttributeNS(null, "fill", "url(#madlo_hor)");
            SVGDocument.getElementById("gunSight").setAttributeNS(null, "display", "block");
            SVGDocument.getElementById("q_SET_BACK").setAttributeNS(null, "fill", "#bb0000");
            SVGDocument.getElementById("q_SET_TEXT").firstChild.data = "HQ";

            SVGDocument.getElementById("q_WINGS_FLY").setAttributeNS(null, "display", "block");
            SVGDocument.getElementById("q_SUP_ITSELF").setAttributeNS(null, "mask", "url(#supWings)");
            SVGDocument.getElementById("q_TAIL_FLY").setAttributeNS(null, "display", "block");
            SVGDocument.getElementById("q_KOT_ITSELF").setAttributeNS(null, "mask", "url(#catTail)");
        } else {
            SVGDocument.getElementById("q_SKY").setAttributeNS(null, "fill", "#FF9D00");
            SVGDocument.getElementById("q_STREET").setAttributeNS(null, "fill", "#FAE773");
            SVGDocument.getElementById("q_HEALTH").setAttributeNS(null, "fill", "#198C00");
            SVGDocument.getElementById("q_BABKA").endElement();
            SVGDocument.getElementById("q_SUN").endElement();
            SVGDocument.getElementById("q_FILLTOMADLO").setAttributeNS(null, "fill", "#9c4931");
            SVGDocument.getElementById("q_MADLOHOR").setAttributeNS(null, "fill", "#9c4931");
            SVGDocument.getElementById("gunSight").setAttributeNS(null, "display", "none");
            SVGDocument.getElementById("q_SET_BACK").setAttributeNS(null, "fill", "green");
            SVGDocument.getElementById("q_SET_TEXT").firstChild.data = "LQ";

            SVGDocument.getElementById("q_WINGS_FLY").setAttributeNS(null, "display", "none");
            SVGDocument.getElementById("q_SUP_ITSELF").setAttributeNS(null, "mask", "none");
            SVGDocument.getElementById("q_TAIL_FLY").setAttributeNS(null, "display", "none");
            SVGDocument.getElementById("q_KOT_ITSELF").setAttributeNS(null, "mask", "none");
        }
    ...
    function setMusic(event){
        ...
        if (music){
            SVGDocument.getElementById("music").beginElement();

            SVGDocument.getElementById("q_MUSIC_BACK").setAttributeNS(null, "fill", "navy");
            SVGDocument.getElementById("q_MUSIC_SKRT").setAttributeNS(null, "visibility", "hidden");
        } else {
            SVGDocument.getElementById("music").endElement();

            SVGDocument.getElementById("q_MUSIC_BACK").setAttributeNS(null, "fill", "#bb00bb");
            SVGDocument.getElementById("q_MUSIC_SKRT").setAttributeNS(null, "visibility", "visible");
        }
    ...
    function afterDeath(){
        if (newGameAllowed){
            SVGDocument.getElementById("hideDeathScreen").beginElement();
            if (score>1){
                SVGDocument.getElementById("showSignatureScreen").beginElement();
            } else {
                showScoreList(false);
            }
        }
    ...
    function abeceda(evt){
        ...
        var abeceda = SVGDocument.getElementById("abeceda");

        var group = SVGDocument.getElementById("textCovers");

        var lett = svgDocument.createElementNS(svgns, "text");
        ...
        lett.setAttributeNS(null, "font-size", 24);
        ...
        while (letter < abc.length){
            ...
            // visible bullet
            var bull = svgDocument.createElementNS(svgns, "use");
            ...
            var lettSpan = svgDocument.createElementNS(svgns, "tspan");
            //lettSpan.setAttributeNS(null, "id", "lety-" + abc[letter]);
            lettSpan.setAttributeNS(null, "x", x+11.5);
            ...
            // hovering circle
            var env = svgDocument.createElementNS(svgns, "circle");
    ...
    function getLetter(evt){
        ...
        // show the name
        SVGDocument.getElementById("playerName").firstChild.data = playerName;
    }
    ...
    function showScoreList(withNewUploaded){
        ...
        if (withNewUploaded){
            ...
            if (playerName && score) {
                ...
                var urlLink = "cowboys_score.php?name="+URLEncode(playerName)+"&score="+score;
                // do save of new one and get list again
                getURL(urlLink, callback);
            }
        } else {
            // simply get list
            getURL("cowboys_score.php", callback);
        }

        // do some action of getting new score list + show it
        SVGDocument.getElementById("hideDeathScreen").beginElement();
        SVGDocument.getElementById("hideSignatureScreen").beginElement();
        SVGDocument.getElementById("showHighScoreScreen").beginElement();
    }

    // hide score list and get from beginning
    function hideScoreList(){
        SVGDocument.getElementById("hideHighScoreScreen").beginElement();
        SVGDocument.getElementById("showStartScreen").beginElement();
        if (music)
            SVGDocument.getElementById("music").beginElement();
    }
    ...
    function callback(urlRequestStatus) {
        ...
        if(urlRequestStatus.success) {
            ...
            while (fame.getChildNodes().length>0) {
                fame.removeChild(fame.getFirstChild());
            }
            ...
            for (var y = 0; y < topTwenty; y++) {
                for (var i = 0; i < HallOfFame.length; i++) {
                    if ((HallOfFame[i][0] == tmpArray[y]) && (total<topTwenty)) {
                        // create text span
                        var fameSpan = svgDocument.createElementNS(svgns, "tspan");
